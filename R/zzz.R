.onLoad <- function(libname="FACsim", pkgname="FACsim"){
  options(digits=4)
  library(ggplot2)
  theme_def <- function(base_size = 12, base_family = "") {
    ...
    theme(
      ...
      axis.title=          element_text(size = rel(0.8), vjust=0.35),
      ...
      axis.title.x =       element_text(size = base_size*1.8, vjust = 0.3,
                                        margin = margin(t = 10, b = 0.8 * half_line/2)),
      axis.title.x.top =   element_text(margin = margin(b = half_line), vjust = 0),
      axis.title.y =       element_text(size = base_size*1.8, angle = 90, vjust = 1,
