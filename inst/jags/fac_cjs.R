...
cat("
    ...
    ### Migration survival
      ...
      mean.phi.spr  ~ dbeta(3,2)#dunif(0, 1)               # Mean spring survival
      lmean.PHI.spr <- log(mean.phi.spr / (1 - mean.phi.spr))     # Logit mean spring survival


    ### Annual variation
      for (t in 1:(nYears)){
        epsilon.phi.spr[t] ~ dnorm(0, tau.phi.spr)T(-10, 10)      # Annual variation in spring survival
        epsilon.phi.aut[t] ~ dnorm(0, tau.phi.aut)T(-10, 10)      # Annual variation in fall survival
      }
    ...
    ### Migration survival
      for (t in 1:nYears){
        logit(PHI.aut[t]) <- lmean.PHI.aut + epsilon.phi.aut[t]  # Fall migration survival
        logit(PHI.spr[t]) <- lmean.PHI.spr + epsilon.phi.spr[t]  # Spring migration survival
      } # t
